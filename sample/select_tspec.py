...
def fetch_tspec():
    """
    Searches the database for all ARC TRIPLESPEC
    and returns the object names and 

    """

    # Select all spex spectra from the database
    all_tspec = db.dict.execute("SELECT s.obs_date, s.filename, s.wavelength,"
        " so.shortname, s.source_id FROM spectra AS s JOIN sources AS so ON "
        "so.id=s.source_id WHERE s.instrument_id=16").fetchall()

    arc_tspec = [[i['shortname'],i['filename'],i['source_id']] 
                 for i in all_tspec]

    return arc_tspec

def make_tspec_batch(model_name="marley",model_file="SXD_Marley.pkl"):

    logging.debug("model {} file {}".format(model_name,model_file))

    arc_tspec = fetch_tspec()
    logging.debug("Got Sample")

    h = open('submit_all_{}.sh'.format(model_name),'w')
    for name,filename,sid in arc_tspec:
        if name==None:
...
def get_source_info():
    ...
    outfile = "/home/stephanie/ldwarfs/TSPEC_info.dat"
    f = open(outfile,"w")
...
#make_tspec_batch("Marley","SXD_r2000_Marley.pkl")
#make_tspec_batch("BTSettl","SXD_r2000_BTS.pkl")
#make_tspec_batch("B06_","B06_cloud_exp_r1200.pkl")
make_tspec_batch("Dusty","SXD_r2000_Dusty.pkl")
