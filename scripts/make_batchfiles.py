...
h = open('submit_all_dusty.sh','w')
for unum in unums:
    h.write('qsub run_dusty_{}.sh\n'.format(unum))

    f = open('run_dusty_{}.py'.format(unum),'w')
    ...
    f.write("ob = OneBatch('{}','/vega/astro/users/sd2706/modelSpectra/SpeX_dusty.pkl')\n".format(unum))
    f.close()

    g = open('run_dusty_{}.sh'.format(unum),'w')
    g.write("#!/bin/sh\n\n")
    g.write("# Directives\n")
    g.write("#PBS -N Marley{}\n".format(unum))
    g.write("#PBS -W group_list=yetiastro\n")
    g.write("#PBS -l nodes=1,walltime=10:00:00,mem=4000mb\n")
    ...
    g.write("/vega/astro/users/amp2217/yt-x86_64/bin/python run_dusty_{}.py\n".format(unum))
