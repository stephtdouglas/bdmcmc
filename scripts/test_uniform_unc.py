...
snr = bd.specs['low']['flux']/bd.specs['low']['unc']

high_snr = np.where(snr>10)[0]
max_good = max(bd.specs['low']['unc'][high_snr])

bd.specs['low']['unc'][high_snr] = max_good
...
bdsamp.plot_all(outfile='test_uniform_unc_{}.pdf'.format(date.isoformat(date.today())))
