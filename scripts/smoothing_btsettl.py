...
mods = BDdb.get_db(modelpath+'model_atmospheres.db')
...
for i in range(d):
    ...
    logging.debug('{} fsyn {}'.format(i, len(models['fsyn'][i])))
    models['wsyn'].append(np.asarray(x[0]['wavelength'][nir])*u.um)
...
for l in loggs:
    for t in teffs:
        ...
        if (len(l_i)>0) and (len(lup_i)>0) and (len(ldn_i)>0):
            w_orig = models['wsyn'][l_i]
            w_orig_up = models['wsyn'][lup_i]
            w_orig_dn = models['wsyn'][ldn_i]
