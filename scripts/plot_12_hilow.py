...
ax.set_ylable('flux (normalized at 1.27 microns)')
for u in unums:
    plot_low(ax,bds[u],offset,'{} {}'.format(bds[u].spt,u))
    print u
    if u==unums[3]:
        ax = plt.subplot(132)
        ax.set_ylim(-0.1,offset_start+1.2)
...
for order in np.asarray([58,59,61,62,63,64,65]):
    ...
    ax.set_ylim(-0.2,offset_start+1.5)
    ...
    for u in unums:
        ...
        if u==unums[5]:
            ax = plt.subplot(122)
            ax.tick_params(labelsize='large')
            ax.set_ylim(-0.3,offset_start+1.5)
